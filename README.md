...
- קלט מקובצי אודיו ווידאו כאחד — הקול מחולץ אוטומטית
- חותמות זמן לכל קטע, עם מתג להצגה או להסתרה
- ניקוי מילות מילוי בעברית (אה, אממ, כאילו, יעני…) עם רשימה שניתן לערוך (עם אייקון **i** להסבר)
- רשימת מונחים ושמות, כדי שהמודל יזהה נכון שמות פרטיים ומושגים מקצועיים (עם אייקון **i** להסבר)
...
- 8 GB זיכרון (מספיק להקלטות של שעתיים–שלוש)
...
| סוג | פורמטים |
|------|----------|
| אודיו נפוץ | `mp3`, `wav`, `m4a`, `aac` |
| אודיו דחוס | `flac`, `ogg`, `opus`, `wma` |
| וידאו (הקול מחולץ אוטומטית) | `mp4`, `mkv`, `mov`, `avi`, `webm` |

כל הפורמטים בטבלה נבדקו מקצה לקצה — תמלול, הפרדת דוברים ושמירת הקבצים.
בפועל נתמך כמעט כל פורמט ש־ffmpeg יודע לפתוח, כי הוא מנוע הפענוח שמתחת.

חלון בחירת הקובץ מציג כברירת מחדל אודיו ווידאו יחד. אפשר לצמצם לסוג אחד,
או לבחור **All files** לכל פורמט אחר, דרך הבורר שבתחתית החלון.

### אורך ההקלטה

אין הגבלת גודל. מה שקובע הוא **אורך** ההקלטה ולא משקל הקובץ — שעה ב־`mp3`
ושעה ב־`wav` צורכות אותו זיכרון אחרי הפענוח.

| מדד | לשעת הקלטה |
|------|-------------|
| זמן עיבוד (מודל `small`, GPU) | כ־17 דקות |
| זיכרון | כ־220 מגה, מעל בסיס של כ־1 ג'יגה |

הקלטות של שעתיים–שלוש עוברות בנוחות על מחשב עם 8 ג'יגה זיכרון.
מעבר לכך מומלץ לפצל את ההקלטה, וגם זמן העיבוד כבר נעשה משמעותי.
...
| מונחים ושמות | ברירת מחדל | רשימה שניתן לערוך · **i** להסבר |
| מילות מילוי | ברירת מחדל | רשימה שניתן לערוך · **i** להסבר |
| חותמות זמן | מתג בסרגל התמלול | מתג בסרגל התמלול |

<div dir="rtl" align="right">

### מונחים ושמות · מילות מילוי

במצב **מקצועי**, ליד כל אחת מהרשימות מופיע אייקון **i** עגול כחול. העברת העכבר עליו
מציגה הסבר קצר על השדה (כמו בצילום המסך למעלה).

- **מונחים ושמות** — שמות ומונחים שהמודל נוטה לטעות בהם (למשל שמות אנשים,
  חברות, מוצרים). שורה אחת לכל מונח.
- **מילות מילוי** — מילים וצלילי היסוס שיוסרו מהתמלול. שורה אחת לכל מילה.
...
הספרייה הגרפית שבבסיס האפליקציה מציירת שורה בעברית נכון רק כל עוד היא נשארת
חתיכה אחת, וסמן או סימון באמצע שורה מפצלים אותה ומהפכים את סדר המילים על המסך.
לכן בתיבת התמלול הסימון בעכבר מנוטרל (יש כפתור **העתק**), וברשימות **מונחים ושמות**
ו**מילות מילוי** לחיצה מציבה את הסמן בסוף השורה שנלחצה. מקלידים ומוחקים בסוף השורה,
שורה לכל מונח, ו־`Ctrl+A` עדיין מסמן הכול.

## אינטרנט

- **בהתקנה ובהפעלה הראשונה** — נדרש אינטרנט כדי להוריד את מודל התמלול (פעם אחת).
- **לאחר מכן** — התמלול עצמו עובד **מקומית**, גם בלי חיבור. האודיו לא נשלח לשום שרת.
...
כרגע ההתקנה דורשת **`Install-TapeInk.bat`**, שמתקין Python, ffmpeg וספריות
דרך הסקריפט. זה עובד, אבל משתמש לא טכני רואה קבצים, PowerShell ו־SmartScreen.

**החלופה הבאה** היא קובץ **`TapeInkSetup.exe`** — מתקין Windows רגיל:

| | `Install-TapeInk.bat` (היום) | `TapeInkSetup.exe` (עתידי) |
|---|---|---|
| מה המשתמש רואה | תיקיית פרויקט + BAT | אשף התקנה / קיצור דרך |
| Python | מותקן דרך winget | **מובנה בתוך החבילה** — המשתמש לא מתקין |
| ffmpeg | מותקן דרך winget | נארז ביחד או יורד בהתקנה |
| מודל Whisper | יורד בהתקנה | יורד בהפעלה ראשונה (או נארז — קובץ גדול) |
| גודל משוער | ~3 GB (כולל Python) | ~1–3 GB (תלוי אם המודל בפנים) |
| SmartScreen | על BAT/PS1 | גם על EXE (פחות מורגש עם חתימה דיגיטלית) |

**איך בונים את זה:** PyInstaller (או cx_Freeze) לארוז את `app.py` + Python +
ספריות לתיקייה אחת, ו־Inno Setup / NSIS ליצור מתקין `.exe`. זה שלב נפרד
מהפיתוח הנוכחי — לא חוסם שימוש, אבל משפר את חוויית ההפצה למשתמשים
שלא מכירים Python.

> **GitHub Release** (שלב נפרד): בדף הפרויקט ב־GitHub אפשר לפרסם "גרסה רשמית"
> (למשל `v0.1.0`) עם קובץ ZIP או EXE להורדה — כמו דף "הורדות" רשמי, במקום
> שמישהו יצטרך ללחוץ Code → Download ZIP בעצמו.
